  ...
  return (
    ...
    >
      ...
      <View className="px-6 pt-8">
        ...
        <Card variant="info" className="mt-8">
          ...
          {[
            ...
            <View key={tip} className="mb-2 flex-row gap-3">
              <Text className="text-blue-400 font-semibold">{i + 1}.</Text>
              <Text className="flex-1 font-body text-sm text-muted leading-relaxed">{tip}</Text>
            </View>
          ))}
        </Card>
          ...
          {leaderboard.length === 0 ? (
            ...
            leaderboard.map((m) => (
              <View key={m.id} className="flex-row items-center gap-4 border-b border-border py-4 last:border-0">
                <View className="h-10 w-10 items-center justify-center rounded-full flex-shrink-0" style={{ backgroundColor: `${m.color}22`, borderColor: `${m.color}44`, borderWidth: 1.5 }}>
                  <Text className="font-display text-base font-bold" style={{ color: m.color }}>{m.name[0]}</Text>
                </View>
          ...
          <Card>
            ...
            {resolvedLocks.map((lock, idx) => (
              <View key={`${lock.name}-${idx}`} className="flex-row items-center gap-4 border-b border-border py-4 last:border-0">
                <View className="h-10 w-10 items-center justify-center rounded-full flex-shrink-0" style={{ borderWidth: 1.5, borderStyle: lock.fuel ? 'solid' : 'dashed', borderColor: lock.fuel ? `${lock.color}aa` : '#3b3b48', backgroundColor: lock.fuel ? `${lock.color}22` : 'transparent' }}>
                  <Text style={{ color: lock.color }} className="font-display text-base font-bold">{lock.initial}</Text>
                </View>
                <View className="flex-1">
                  {lock.fuel ? (
                    <>
                    ...
                    <>
                      ...
                      <Text className="mt-1 font-mono text-xs text-muted">Current best: 158.9¢/L at Castle Hill</Text>
                    </>
                  )}
                </View>
        ...
        <Button variant="accent" size="lg" fullWidth accessibilityLabel="Invite group member" onPress={() => router.push('/modal/invite')} className="mt-8">
          ＋ Invite Member
        </Button>
      </View>
    </ScrollView>
